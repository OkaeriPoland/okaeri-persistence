...
public interface Persistence<T> extends Closeable {
   ...
   *
   * <p>Mainly for the filesystem or in-memory persistence backends. Not expected to be a guarantee,
   * just something to use when performing mass changes and hoping for the implementation to take
   * care of it.
   *
   ...
   */
  Set<PersistencePath> findMissingIndexes(
      PersistenceCollection collection, Set<IndexProperty> indexProperties);

  /**
   * Prefix for the storage operations.
   *
   * <p>For example: - Redis: as key prefix "{basePath}:{collection}:{id}" - MariaDB: in table name
   * "{basePath}_{collection}"
   *
  ...
  /**
   * Check existence of an object in specific path. Important as it is advised that read returns
  ...
  /**
   * Fetches entity by path from specific collection. Creates empty entity if no saved entity was
   * found.
   *
   * @param collection Target collection (eg. player)
   * @param path Target entity path (eg. uuid)
   ...
   * @return Saved entities
   */
  Map<PersistencePath, T> readOrEmpty(
      PersistenceCollection collection, Collection<PersistencePath> paths);
