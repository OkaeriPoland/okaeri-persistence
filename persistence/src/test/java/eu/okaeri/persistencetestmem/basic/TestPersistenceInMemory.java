...
public class TestPersistenceInMemory {
  ...
  public void setup() {
    ...
    this.repository =
        RepositoryDeclaration.of(UserRepository.class)
            .newProxy(
                this.persistence, this.collection, TestPersistenceInMemory.class.getClassLoader());
  }
  ...
  public void prepareDb() {
    ...
    for (int i = 0; i < CREATE_USERS; i++) {
      ...
      final User user =
          this.persistence.readOrEmpty(this.collection, PersistencePath.of(uuid)).into(User.class);
  ...
  public void test_repository_delete_all_by_path() {
    ...
    // test real user
    assertEquals(
        1, this.repository.deleteAllByPath(Collections.singletonList(this.lastUser.getId())));
  ...
  public void test_repository_delete_by_path() {
    ...
    assertEquals(CREATE_USERS, this.repository.count());
    // test real user
    assertTrue(this.repository.deleteByPath(this.lastUser.getId()));
    assertEquals(CREATE_USERS - 1, this.repository.count());
  }
