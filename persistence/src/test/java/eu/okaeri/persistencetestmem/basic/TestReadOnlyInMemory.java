...
public class TestReadOnlyInMemory {
    ...
    public void setup() {
        ...
        DocumentPersistence persistence = new ReadOnlyDocumentPersistence(this.memory);
        persistence.registerCollection(this.collection);
        this.repository = RepositoryDeclaration.of(UserRepository.class).newProxy(persistence, this.collection, TestReadOnlyInMemory.class.getClassLoader());
    }
    ...
    public void prepareDb() {
        ...
        for (int i = 0; i < CREATE_USERS; i++) {
            ...
            User user = this.memory.readOrEmpty(this.collection, PersistencePath.of(uuid)).into(User.class);
    ...
    public void test_repository_delete_all_by_path() {
        ...
        assertEquals(CREATE_USERS, this.repository.count());
        // test real user
        assertEquals(0, this.repository.deleteAllByPath(Collections.singletonList(this.lastUser.getId())));
        assertEquals(CREATE_USERS, this.repository.count());
    }
    ...
    public void test_repository_delete_by_path() {
        ...
        assertEquals(CREATE_USERS, this.repository.count());
        // test real user
        assertEquals(false, this.repository.deleteByPath(this.lastUser.getId()));
        assertEquals(CREATE_USERS, this.repository.count());
    }
